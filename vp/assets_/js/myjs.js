$(function(){
	...
	function updatecsrftoken(obj, newtoken){
		$("#"+obj+" input[name~='csrf_test_name']").val(newtoken);
	}
	$('body').on('click', '.patwariIDActiveInactive', function(){
		...
		var csrf = $("#frmPatwariList input[name~='csrf_test_name']").val();
		$.ajax({
			type: "POST",
			url: url_,
			...
			success: function(data){
				...
				var newtoken = obj.hash;
				updatecsrftoken('frmPatwari', newtoken);
				updatecsrftoken('frmPatwariList', newtoken);
				updatecsrftoken('frmPatwariUpdate', newtoken);
	...
	$('#frmPatwariUpdate').submit(function(e){
		...
		var csrf = $("#frmPatwariUpdate input[name~='csrf_test_name']").val();
		$('#load_here_edit_actinact').html('<span>Loading <img src="'+base_path+'/assets_/img/load.GIF" width="10" /></span>');
		$.ajax({
			url: url_,
			type: "POST",
			data: {"csrf_test_name": csrf, data_ },
			async: false,
	        cache: false,
	        contentType: false,
	        processData: false,
	...
	$('#frmPatwari').submit(function(e){
		...
		var csrf = $("#frmPatwari input[name~='csrf_test_name']").val();
		$.ajax({
			url:url_,
			type: "POST",
			data:  {data_, "csrf_test_name": csrf },
			async: false,
	        cache: false,
	        contentType: false,
	        processData: false,
